...
def batch_gradient_descent_mnist(
    path: str, c1: int, c2: int, num_epochs: int, learning_rate: float, noprint: bool
) -> float:
    """The batch gradient descent algorithm implemented for MNIST and binary classification.

    Args:
        path (str): Path to the folder holding the MNIST dataset
...
def main():
    ...
    arg_parser.add_argument(
        "path", help="Path to MNIST dataset (will download if needed)", type=str
    )
    ...
    arg_parser.add_argument(
        "lr", type=float, help="Learning rate to use while training."
    )
    arg_parser.add_argument(
        "--noprint", action="store_true", help="Disable printing each epoch."
    )
    ...
    final_accuracy = batch_gradient_descent_mnist(
        args.path, args.class1, args.class2, args.epochs, args.lr, args.noprint
    )
